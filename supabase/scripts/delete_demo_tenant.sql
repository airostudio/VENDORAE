...
-- "default-store"), so deleting the tenant row would break the
...
update tenants set name = 'Your Store' where slug = 'default-store';
...
-- wishlist_items, etc.
delete from products where tenant_id = (select id from tenants where slug = 'default-store');
...
where tenant_id = (select id from tenants where slug = 'default-store')
  and parent_id is not null;
delete from categories where tenant_id = (select id from tenants where slug = 'default-store');

delete from banners where tenant_id = (select id from tenants where slug = 'default-store');
delete from blog_posts where tenant_id = (select id from tenants where slug = 'default-store');
