 ...
 */
export default function Footer({ categories, brandName }: { categories: NavCategory[]; brandName: string }) {
  const columns = [
    ...(categories.length > 0
  ...
  return (
    <footer className="border-t border-stone-200 bg-ink-950 text-warm-100 mt-24">
      <div className="container-page py-16 grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          <p className="font-serif text-2xl mb-4">{brandName}</p>
          <p className="text-sm text-stone-400 leading-relaxed max-w-xs">Quality goods, shipped to your door.</p>
        </div>
        {columns.map((col) => (
          <div key={col.title}>
